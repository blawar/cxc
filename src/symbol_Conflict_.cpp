...
fprintf(debug, ")\n");
						...
						n->isTemplate = false;
						if(symbols[i]->parent /*&& symbols[i]->parent->parent*/) n->index = symbols[i]->parent->functionCount++;
...
string symbol::render(unsigned char showref, symbol* classBase, symbol* root)
{
	string buffer;
...
string& symbol::render(string &buffer, unsigned char showref, symbol* classBase, symbol* root)
{
...
void symbol::parse(STRINGS & tokens, unsigned long &i)
{
	...
	if(isFunction || type == STRUCT || type == CONTROL || type == IMPORT && type != STATEMENT)
	{
		...
		else if(isFunction)
		{
			...
			if(tokens.compare(i, "(")) // if not an open paren, expecting datatype
			{
				...
				if(datatype && datatype->type == STRUCT)
				{
					pointer++;
				}
	...
	else // statement
	{
		...
		if(s && s->isDatatype()) // variable declaration
		{
			...
			if(datatype->type == STRUCT) pointer++;
